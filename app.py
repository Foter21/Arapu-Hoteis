...
def dashboard():
    ...
    try:
        ...
        # =====================================================
        # RESERVAS DO MÊS E ANO SELECIONADOS
        # =====================================================
        ...
        cursor.execute("""
            ...
            INNER JOIN hoteis ht
                ON q.id_hotel = ht.id_hotel
            WHERE MONTH(r.checkin_previsto) = %s
              AND YEAR(r.checkin_previsto) = %s
        ...
        cursor.execute("""
            SELECT DISTINCT
                YEAR(checkin_previsto) AS ano
            FROM reservas
            WHERE checkin_previsto IS NOT NULL
            ORDER BY ano DESC
        """)

        anos_resultado = cursor.fetchall()

        anos_disponiveis = [
            item["ano"]
            for item in anos_resultado
        ...
        if mes_selecionado == 12:
            ...
            primeiro_dia_proximo_mes = datetime(
                ano_selecionado + 1,
                1,
                1
            )
        ...
        pagamentos_por_dia = cursor.fetchall()

        faturamento_por_data = {}

        for item in pagamentos_por_dia:
            ...
            faturamento_por_data[data_pagamento] = float(
                item["total"] or 0
            )
        ...
        quantidade_dias = (
            primeiro_dia_proximo_mes - primeiro_dia_mes
        ).days
        ...
        for numero_dia in range(quantidade_dias):
            ...
            valor = faturamento_por_data.get(
                data_atual,
                0
            )

            faturamento_grafico.append({
        ...
        maior_valor = max(
            (
                item["valor"]
                for item in faturamento_grafico
            ),
            default=0
        )
        ...
        if maior_valor <= 0:
            ...
            escala_grafico = 10000

        elif maior_valor <= 20000:
            escala_grafico = 20000

        else:
            escala_grafico = (
                (int(maior_valor / 10000) + 1)
                * 10000
            )

        # =====================================================
        # PERCENTUAL DE CADA BARRA
        # =====================================================
        ...
        for item in faturamento_grafico:
            ...
            if escala_grafico > 0:
                ...
                item["percentual"] = round(percentual, 2)
        ...
        faturamento_escala = {
            "topo": escala_grafico,
            "setenta_cinco": escala_grafico * 0.75,
            "cinquenta": escala_grafico * 0.50,
            "vinte_cinco": escala_grafico * 0.25,
            "zero": 0
        }
        ...
        # =====================================================
        # FATURAMENTO DOS PRÓXIMOS 7 DIAS
        # =====================================================
        ...
        return render_template(
            ...
            faturamento_escala=faturamento_escala
        )
...
def pagina_usuarios():
    ...
    try:
        ...
        resultado = cursor.fetchone()
        administradores = resultado["total"] if resultado else 0
        ...
        resultado = cursor.fetchone()
        gerentes = resultado["total"] if resultado else 0
        ...
        resultado = cursor.fetchone()
        recepcao = resultado["total"] if resultado else 0

        return render_template(
            "pagina_usuarios.html",
            nome=session.get("nome", "Usuário"),
            perfil=session.get("perfil", "Não informado"),
            id_hotel=session.get("id_hotel"),
        ...
        print(f"Erro ao carregar página de usuários: {erro}")
        return f"Erro ao carregar página de usuários: {erro}", 500

    except Exception as erro:
        print(f"Erro inesperado nos usuários: {erro}")
        return f"Erro inesperado nos usuários: {erro}", 500
